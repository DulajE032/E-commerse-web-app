      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start relative">
          ...
          `}>
             ...
             {/* 1. Category Filter */}
             <div className="mb-8">
                <h3 className="font-bold text-slate-900 mb-4 uppercase tracking-widest text-xs">Category</h3>
                ...
                <div className="space-y-3">
                  {brands.map(brand => (
                    <label key={brand} className="flex items-center gap-3 cursor-pointer group">
                      <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${selectedBrands.includes(brand) ? 'bg-slate-900 border-slate-900' : 'border-gray-300 bg-white group-hover:border-slate-500'}`}>
            ...
            <div className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between overflow-x-auto">
              <div className="flex items-center gap-4 px-3 min-w-max">
                ...
                <div className="flex gap-2">
                  {Object.keys(sortOptions).slice(0, 4).map(option => (
                     <button 
                       key={option}
                ...
                products.map((product, idx) => (
                  ...
                  >
                    ...
                    <div className="flex-1 flex flex-col">
                      <p className="text-slate-500 text-[10px] md:text-xs font-bold uppercase tracking-wider mb-1">{product.category}</p>
                      <h3 className="text-slate-900 font-bold text-sm leading-snug mb-2 line-clamp-2 group-hover:text-slate-600 transition-colors">{product.name}</h3>
